        ...
        if user is None:
            ...
            if profile and profile.institution_type != institution_type:
                ...
                if profile.institution_type == 'tertiary':
                    return redirect('tertiary_dashboard', role=profile.role)
                return redirect('secondary_dashboard', role=profile.role)
            else:
...
def secondary_login(request):
    return _login_for_institution(request, 'secondary', 'core/secondary_login.html')


def tertiary_login(request):
    return _login_for_institution(request, 'tertiary', 'core/tertiary_login.html')
...
def _get_secondary_nav(role):
    ...
    for item_id in role_map.get(role, []):
        url = f"/secondary/{role}/" if item_id == 'dashboard' else f"/secondary/{role}/{item_id}/"
        items.append({
            'id': item_id,
...
def _get_tertiary_nav(role):
    ...
    items = []
    for item_id in role_map.get(role, []):
        url = f"/tertiary/{role}/" if item_id == 'dashboard' else f"/tertiary/{role}/{item_id}/"
        items.append({
            'id': item_id,
            'label': labels.get(item_id, item_id.replace('-', ' ').title()),
            'icon': icons.get(item_id, 'layout-dashboard'),
...
def secondary_admin_register(request):
    ...
    return render(request, 'core/secondary_admin_register.html', {
...
def secondary_create_user(request):
    ...
    if not creator_profile or creator_profile.institution_type != 'secondary' or creator_profile.role != 'admin':
        ...
        return redirect('secondary_dashboard', role=creator_profile.role if creator_profile else 'admin')
    ...
    if request.method == 'POST':
        ...
        if not all([password, full_name]):
            messages.error(request, 'Please fill all required fields.')
            return redirect('secondary_dashboard', role=creator_profile.role)
...
def secondary_add_class(request):
    ...
    return redirect('secondary_page', role=creator_profile.role, page='classes')
...
def secondary_create_fee(request):
    ...
    if not creator_profile or creator_profile.institution_type != 'secondary' or creator_profile.role != 'accountant':
        ...
        return redirect('secondary_dashboard', role=creator_profile.role if creator_profile else 'accountant')
    ...
    if request.method == 'POST':
        ...
        if not name or not amount:
            ...
            return redirect('secondary_page', role=creator_profile.role, page='fees')
...
def secondary_save_scores(request):
    ...
    if not profile or profile.institution_type != 'secondary' or profile.role != 'teacher':
        ...
        return redirect('secondary_dashboard', role=profile.role if profile else 'teacher')
    ...
    if not staff:
        ...
        return redirect('secondary_page', role=profile.role, page='enter-scores')
...
def secondary_submit_results(request):
    ...
    if not staff:
        messages.error(request, 'Teacher record not found.')
        return redirect('secondary_page', role=profile.role, page='submit-results')
...
def secondary_review_results(request):
    ...
    if not profile or profile.institution_type != 'secondary' or profile.role != 'examiner':
        ...
        return redirect('secondary_dashboard', role=profile.role if profile else 'examiner')
    ...
    if request.method == 'POST':
        ...
        if not submission:
            ...
            return redirect('secondary_page', role=profile.role, page='review-results')
        submission.status = 'reviewed'
        submission.save(update_fields=['status'])
        messages.success(request, 'Results marked as reviewed.')
        return redirect(f"/secondary/{profile.role}/review-results/?submission={submission.id}")
...
def tertiary_create_fee(request):
    ...
    if not creator_profile or creator_profile.institution_type != 'tertiary' or creator_profile.role != 'accountant':
        messages.error(request, 'Only Accountants can manage fees.')
        return redirect('tertiary_dashboard', role=creator_profile.role if creator_profile else 'accountant')
    ...
    if request.method == 'POST':
        ...
        if not name or not amount:
            messages.error(request, 'Fee name and amount are required.')
            return redirect('tertiary_page', role=creator_profile.role, page='fees')
...
def secondary_add_session_term(request):
    ...
    if request.method == 'POST':
        ...
        if not session_name or not term:
            ...
            return redirect('secondary_page', role=creator_profile.role, page='sessions')
...
def secondary_add_student(request):
    ...
    if request.method == 'POST':
        ...
        if not full_name or not password:
            ...
            return redirect('secondary_page', role=creator_profile.role, page='register')
...
def secondary_assign_teacher(request):
    ...
    return redirect('secondary_page', role=creator_profile.role, page='assign-teachers')
...
def secondary_add_subject(request):
    ...
    return redirect('secondary_page', role=creator_profile.role, page='subjects')
...
def secondary_add_teacher(request):
    ...
    if not creator_profile or creator_profile.institution_type != 'secondary' or creator_profile.role not in ['admin', 'registry']:
        ...
        return redirect('secondary_dashboard', role=creator_profile.role if creator_profile else 'registry')
    ...
    if request.method == 'POST':
        ...
        if not full_name or not password:
            messages.error(request, 'Full name and password are required.')
            return redirect('secondary_page', role=creator_profile.role, page='users')
...
@login_required(login_url='/tertiary/login/')
def tertiary_create_user(request):
    ...
    if not creator_profile or creator_profile.institution_type != 'tertiary' or creator_profile.role not in ['vc', 'provost']:
        ...
        return redirect('tertiary_dashboard', role=creator_profile.role if creator_profile else 'vc')
    ...
    if request.method == 'POST':
        ...
        if not all([password, full_name]):
            messages.error(request, 'Please fill all required fields.')
            return redirect('tertiary_dashboard', role=creator_profile.role)
...
def approve_profile(request, profile_id):
    ...
    if approver.institution_type == 'secondary' and approver.role != 'admin':
        ...
        return redirect('secondary_dashboard', role=approver.role)
    ...
    if approver.institution_type == 'tertiary' and approver.role not in ['vc', 'provost']:
        ...
        return redirect('tertiary_dashboard', role=approver.role)
...
def secondary_page(request, role, page):
    ...
    return render(request, 'core/secondary_page.html', {
        'role': role,
...
def tertiary_page(request, role, page):
    ...
    return render(request, 'core/tertiary_page.html', {
        'role': role,
        'role_label': role_label,
        'nav_items': nav_items,
        'active_page': page,
